...
module.exports = {
    ...
    async execute(interaction) {    
        ...
        if(!map.get(channel.guild.id)) {
            ...
            if(interaction.options._hoistedOptions.length < 1) return interaction.editReply("You need to provide a query!")
            ...
            if(interaction.options._hoistedOptions.length < 1) {
                ...
                if(!connection || connection._state.subscription.player._state.status == "idle") return interaction.editReply({content: "There is nothing to unpause!"})

                // Check if player is playing
                if(connection._state.subscription.player._state.status == "playing") return interaction.editReply("Already playing!")

                // Check if the member is in the same vc as the bot
                if(interaction.member.voice.channel != getVoiceConnection(interaction.guild.id).joinConfig.channelId) return interaction.editReply("You need to be in the same voice channel as the bot!")
